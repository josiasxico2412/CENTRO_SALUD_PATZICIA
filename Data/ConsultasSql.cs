...
namespace CENTRO_SALUD_PATZICIA.Data
{
	public class ConsultasSql
	{
		...
		public static List<ModeloObtenerEncabezado> ObtenerEncabezado()
		{
			List<ModeloObtenerEncabezado> resultadoConsulta = new List<ModeloObtenerEncabezado>();
			...
			string consulta = @"SELECT TOP 1 
								...
								Cargo,
								FORMAT(Mes, 'MMMM') AS Mes,
								YEAR(Anio) AS Anio,
								Firma
							FROM EncabezadoRegistro
							ORDER BY ID_EncabezadoRegistro DESC;";
				...
				using (SqlConnection connection = new SqlConnection(cadenaConexion))
				{
					...
					using (SqlCommand command = new SqlCommand(consulta, connection))
					{
						using (SqlDataReader reader = command.ExecuteReader())
						{
							while (reader.Read())
							{
								ModeloObtenerEncabezado registro = new ModeloObtenerEncabezado()
								{
									Area_Salud = reader["Area_Salud"].ToString(),
									DistritoSalud = reader["DistritoSalud"].ToString(),
									Municipio = reader["Municipio"].ToString(),
									Servicio_Salud = reader["Servicio_Salud"].ToString(),
									ResponsableInformacion = reader["ResponsableInformacion"].ToString(),
									Cargo = reader["Cargo"].ToString(),
									Mes = reader["Mes"].ToString(),
									Anio = Convert.ToInt32(reader["Anio"].ToString()),
